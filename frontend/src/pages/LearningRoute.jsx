...
export default function LearningRoute({ selectedModule, setSelectedModule, refreshDashboard }) {
  ...
  const submitSurvey = async () => { await api.saveSurvey(module.id, { user_id: 'ana', answers, rating: Number(answers.rating || 5), comments: answers.comments || '' }); await refreshDashboard(); alert('Encuesta guardada'); };
  return <div className="grid grid-2">
    ...
    <div className="card">
      <h3>Conexión con laboratorio local</h3><p className="muted">El módulo se conecta con el escenario Docker asignado, pero la API registra acciones en modo seguro si no habilitas ejecución real.</p>
      <div className="card soft"><FileText/> <strong>Objetivos</strong><ul>{(module.objetivos || []).map(o => <li key={o}>{o}</li>)}</ul></div>
      <div className="card soft"><strong>Checkpoints del ejercicio</strong>{module.checkpoints.map(cp => <p key={cp.id}>• {cp.titulo}</p>)}</div>
    </div>
  </div>;
}
